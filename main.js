var ENDPOINT = 'https://cbxexyflhj.execute-api.us-east-1.amazonaws.com/beta/acorns';

angular.module('app', ['ngRoute', 'ui.ace'])

    .config(['$routeProvider', function($routeProvider) {
        $routeProvider
            .when('/', {
                templateUrl: 'root.html'
            })
            .when('/acorns', {
                templateUrl: 'acorn-list.html',
                controller: 'AcornListController'
            })
            .when('/new', {

                templateUrl: 'edit.html',
                controller: 'EditController'
            })
            .when('/acorns/:acornId', {
                templateUrl: 'acorn.html',
                controller: 'AcornController'
            })
            .when('/acorns/:acornId/edit', {
                templateUrl: 'edit.html',
                controller: 'EditController'
            })
            .otherwise({
                redirectTo: '/'
            });
    }])

    .controller('MainController', ['$scope', '$location', '$http', function($scope, $location, $http) {
        $scope.routeActive = function(loc) {
            return loc === $location.path();
        };

        $scope.loading = false;

        $scope.ready = function() {
            $scope.loading = false;
        };

        $scope.acornREST = function(method, endpoint, data) {
            $scope.loading = true;
            return $http[method](endpoint, data)
                .catch(console.log)
                .finally($scope.ready);
        };
    }])

    .controller('AcornListController', ['$scope', '$location', '$http', function($scope, $location, $http) {
        $scope.visit = function(id) {
            $location.path("acorns/" + id);
        };

        $scope.acornREST('get', ENDPOINT)
            .then(function(response) {
                $scope.acorns = response.data;
            });
    }])

    .controller('EditController', ['$scope', '$location', '$routeParams', '$http', function($scope, $location, $routeParams, $http) {
        $scope.editing = !!$routeParams.acornId;

        if ($scope.editing) {
            $scope.acornREST('get', ENDPOINT + "/" + $routeParams.acornId)
                .then(function(response) {
                    $scope.acorn = response.data.Item;
                });
        } else {
            // We're making a new acorn!
            $scope.acorn = {
                name: 'untitled',
                author: 'anonymous',
                language: 'javascript',
                code: '',
            };
        }

        var patchAcorn = function(acorn) {
            if ($scope.editing) {
                return {
                    "operation": "update",
                    "TableName": "acorns",
                    "Key": {
                        "id": acorn.id
                    },
                    "UpdateExpression": "SET #n = :n, #a = :a, #l = :l, #c = :c",
                    "ExpressionAttributeNames": {
                        "#n": "name",
                        "#a": "author",
                        "#l": "language",
                        "#c": "code"
                    },
                    "ExpressionAttributeValues": {
                        ":n": acorn.name,
                        ":a": acorn.author,
                        ":l": acorn.language,
                        ":c": acorn.code
                    }
                };
            } else {
                return {
                    "operation": "create",
                    "TableName": "acorns",
                    "Item": acorn
                };
            }
        };

        $scope.save = function() {
            var method = $scope.editing ? 'put' : 'post';
            if (!$scope.editing) {
                $scope.acorn.id = CryptoJS.SHA3(JSON.stringify($scope.acorn) + Math.random(), { outputLength: 224 }).toString();
            }
            $scope.acornREST(method, ENDPOINT + "/" + $scope.acorn.id, patchAcorn($scope.acorn))
                .then(function() {
                    $location.path("acorns/" + $scope.acorn.id);
                });
        };
    }])

    .controller('AcornController', ['$scope', '$location', '$routeParams', '$http', function($scope, $location, $routeParams, $http) {
        $scope.run = function() {
            $scope.running = true;
            $http.post(ENDPOINT + "/" + $routeParams.acornId + "/run", $scope.acorn)
                .then(function(response) {
                    data = response.data || '';
                    if (data.hasOwnProperty('errorMessage')) {
                        $scope.output = {
                            err: true,
                            message: data.errorMessage
                        };
                    } else {
                        $scope.output = {
                            err: false,
                            message: data
                        };
                    }
                })
                .catch(console.log)
                .finally(function() {
                    $scope.running = false;
                });
        };

        $scope.delete = function() {
            $scope.acornREST('delete', ENDPOINT + "/" + $routeParams.acornId)
                .then(function() {
                    $location.path('acorns');
                });
        };

        $scope.acornREST('get', ENDPOINT + "/" + $routeParams.acornId)
            .then(function(response) {
                $scope.acorn = response.data.Item;
            });
    }]);
